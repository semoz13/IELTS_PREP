export class PathGenerator {
  ...
  listening = {
    ...
    getAttemptState: (attemptId: string) =>
      this.prepend(`/listening/attempts/${attemptId}`),
    registerPlay: (attemptId: string) =>
      this.prepend(`/listening/attempts/${attemptId}/play`),
    saveAnswer: (attemptId: string) =>
      this.prepend(`/listening/attempts/${attemptId}/answer`),
    submit: (attemptId: string) =>
      this.prepend(`/listening/attempts/${attemptId}/submit`),
  };

  writing = {
    startTest: () => this.prepend("/writing/start"),
    getAttemptState: (attemptId: string) =>
      this.prepend(`/writing/attempt/${attemptId}`),
    submitTask: (attemptId: string) =>
      this.prepend(`/writing/attempt/${attemptId}/submit`),
    reviewSubmission: (submissionId: string) =>
      this.prepend(`/writing/submissions/${submissionId}/review`),
    getPendingReviews: () => this.prepend("/writing/reviews/pending"),
  };

  logs = {
    getAll: () => this.prepend("/logs"),
    getMe: () => this.prepend("/logs/me"),
